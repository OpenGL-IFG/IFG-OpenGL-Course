...
int x,y,old_mx=-1,old_my,mx,my,first_mx,first_my; // deklaracia, itogshi -1 imito ro pirvel daklikebaze xazi ar gaaketos, xo gamige?
int c_clicked = 0;
int run_loop = 1;
...
void doButtonPressEvent(XButtonEvent *pEvent)
{
    if (!c_clicked)
    {
        if (old_mx == -1) // tu pirveli klikia dauyenos old mnishvnelobebs pirveli kilis mnishvneloobebi xolo
        {
            ...
            first_mx = old_mx = pEvent->x; // konkretulad es buton iventidan igebs x da ys sadac mxda dacheris inventi
